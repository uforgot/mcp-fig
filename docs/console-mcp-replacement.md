...
| Comments·annotations | get/post/delete comments, get/set annotations | optional `collaboration` profile의 `figma_collaboration.comments`가 Figma REST를 통해 file comments를 읽고 node/resolution/limit filter를 지원. post/delete와 Plugin annotations는 미구현 | **partial** | P1 | 실제 comment dogfood로 read 요구 확인 | collaboration profile tools/list, REST contract, live target comment read 통과. write/delete는 별도 명시 요청과 destructive safety 설계 후 진행 |
