...
3. 모든 target node가 patch의 각 property를 지원하는지 사전검증한다.
4. 한 node라도 unsupported면 mutation은 0건이다.
5. apply 전에 각 target의 patch 관련 state를 snapshot한다.
6. Figma setter가 중간에 실패하면 모든 target을 snapshot으로 rollback한다.
7. rollback도 실패하면 상태를 성공으로 가장하지 않고 `UNKNOWN_OUTCOME`을 반환한다.
8. 전부 성공한 경우에만 change record를 남기고 exact serialized readback을 반환한다.
